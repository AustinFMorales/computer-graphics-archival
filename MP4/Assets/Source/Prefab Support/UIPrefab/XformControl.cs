...
public class XformControl : MonoBehaviour {
    ...
    public Transform SelectedOrigin;
    ...
    void Update() {
        // pretty inefficient way to do this, but given time constraints, this is our best option
        if (Axis.isOn) {
            SelectedOrigin.gameObject.SetActive(true);
            SelectedOrigin.localPosition = mSelected.localPosition;
            SelectedOrigin.localRotation = mSelected.localRotation;
        } else {
            SelectedOrigin.localRotation = Quaternion.identity;
            SelectedOrigin.localPosition = Vector3.zero;
            SelectedOrigin.gameObject.SetActive(false);
        }
    }
