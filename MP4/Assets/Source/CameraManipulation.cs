...
{
    // Camera Manipulation is taken by Sung's repo in Week 6 Example 8.ShowView + NDC
    ...
    void Start()
    {
        ...
        // SetLookAtPos(LookAt.transform.position);
    }
    ...
    void Update()
    {
        ...
        // however, we are not allowed to use this.
        // transform.LookAt(LookAt.transform.localPosition);

        //  can we can do this without transform.LookAt by simply doing this?
        // can we use this idea instead to emulate closely to transform.LookAt?
        // https://forum.unity.com/threads/difference-between-quaternion-and-lookat.639370/
        
        // NOT ALLOWED TO USE - TRANSFORM.LOOKROTATION() IS A TYPO FOR QUATERNION.LOOKROTATION
        // transform.rotation = Quaternion.LookRotation(LookAt.transform.localPosition - transform.position);

        // we already do the calculation at rotate camera position.
